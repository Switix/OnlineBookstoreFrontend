<template>
    <form @submit.prevent="updateBillingAddress" class="space-y-4">
        <div>
            ...
            <input v-model="billingAddressChange.phoneNumber" type="text" id="phoneNumber" name="phoneNumber" required
        ...
        <div class="flex space-x-4">
            <div class="w-2/5">
                ...
                <input v-model="billingAddressChange.zipCode" type="text" id="zipCode" name="zipCode" required class="p-2 w-full bg-bg border rounded-md focus:ring-1 outline-none border-white 
                      focus:border-primary-200 focus:ring-primary-200">
            </div>
            <div class="w-3/5 relative">
                <label for="cityName" class="block font-semibold">Miejscowość</label>
                <input v-model="searchText" type="text" @focus="isDropdownOpen = true" id="cityName" name="cityName"
                    required class="block w-full p-2 bg-bg border rounded-md focus:ring-1 outline-none" :class="{
...
export default {
    ...
    computed: {
        ...
        billingAddressChange() {
            return {
                id: this.user.billingAddress ? this.user.billingAddress.id : '',
    ...
    methods: {
        handleButtonClick() {
            // Function logic here
            console.log('Button clicked in BillingAddressEdit component');
        },
        async updateBillingAddress() {
            ...
            if (this.billingAddressChange.id != '') {
                await this.$store.dispatch('user/updateBillingAddress', this.billingAddressChange);
            }
            else {
                await this.$store.dispatch('user/createBillingAddress', this.billingAddressChange);
            }
            if (this.redirect) {
                router.go(-1);
            }
